...
#define SQL_Date     @"DATETIME"    //date

#ifdef DEBUG
#define SQLog(...) printf("SQL: %s\n", [[NSString stringWithFormat:__VA_ARGS__] UTF8String])
#else
#define SQLog(...)
#endif
        ...
        while ([set next]) {
            ...
            for (NSString *key in dic) {
                ...
                if ([dic[key] isEqualToString:SQL_TEXT]) {
                    ...
                    if (value)
                        [resultDic setObject:value forKey:key];
                } else if ([dic[key] isEqualToString:SQL_Date]) {
                    NSDate* value= [set dateForColumn:key];
                    if(value){
                        [resultDic setObject:value forKey:key];
                    }
                }
            ...
            while ([set next]) {
                ...
                for (NSString *name in clomnArr) {
                    if ([propertyType[name] isEqualToString:SQL_TEXT]) {
                        ...
                        if (value)
                            [model setValue:value forKey:name];
                    } else if ([propertyType[name] isEqualToString:SQL_Date]) {
                        NSDate* value= [set dateForColumn:name];
                        if(value){
                            [model setValue:value forKey:name];
                        }
                    }
...
- (CGFloat)jq_lookupTable:(NSString *)tableName sum:(NSString *)sum whereFormat:(NSString *)format, ...{
    va_list args;
    va_start(args, format);
    NSString *where = format?[[NSString alloc] initWithFormat:format locale:[NSLocale currentLocale] arguments:args]:format;
    va_end(args);

    NSMutableString *finalStr = [[NSMutableString alloc] initWithFormat:@"select sum(%@) from %@ %@", sum, tableName, where?where:@""];
    SQLog(@"%@",finalStr);
    FMResultSet *set = [_db executeQuery:finalStr];
    CGFloat count = 0.0;
    while ([set next]) {
        count = [set doubleForColumnIndex:0];
    }
    return count;
}
